...
<c:if test="${not empty param.deleteId}">
    <c:set var="result" value="${postDAO.deletePost(param.deleteId)}" />
    <c:redirect url="postslist.jsp" />
</c:if>

<c:set var="currentPage" value="${empty param.page ? 1 : param.page}" />
<c:set var="POSTS_PER_PAGE" value="5" />

<c:set var="categories" value="${postDAO.getAllCategories()}" scope="request" />
<c:set var="authors" value="${postDAO.getAllAuthors()}" scope="request" />

<c:set var="posts" value="${postDAO.getFilteredPosts(param.search, param.sortBy, param.category, param.author, param.dateFilter, param.status, param.feature, currentPage, POSTS_PER_PAGE)}" scope="request" />
<c:set var="totalPosts" value="${postDAO.getTotalFilteredPosts(param.search, param.category, param.author, param.dateFilter, param.status, param.feature)}" />
<c:set var="totalPages" value="${(totalPosts + POSTS_PER_PAGE - 1) / POSTS_PER_PAGE}" />

<c:set var="search" value="${param.search}" scope="request" />
<c:set var="sortBy" value="${param.sortBy}" scope="request" />
<c:set var="category" value="${param.category}" scope="request" />
<c:set var="author" value="${param.author}" scope="request" />
<c:set var="dateFilter" value="${param.dateFilter}" scope="request" />
<c:set var="status" value="${param.status}" scope="request" />
<c:set var="feature" value="${param.feature}" scope="request" />
<c:set var="checkNull" value="${totalPages}" scope="request" />
                        ...
                        <div class="wc-title">
                            ...
                            <div class="top-bar">
                                <div class="search-container">
                                    <form action="postslist.jsp" method="GET" class="d-flex">
                                    ...
                                    <a href="addpost.jsp" class="btn btn-warning btn-add-post">Add Post</a>
                                </div>
                            </div>

                            <!-- Filters -->
                            <div class="filters">
                                <form action="postslist.jsp" method="GET" class="d-flex align-items-center text-black" style="color: black;">
                                    ...
                                    <a href="postslist.jsp" class="btn btn-warning reset">Reset Filters</a>
                                </form>
                            </div>
                        </div>
                                        ...
                                        <c:forEach var="post" items="${posts}" varStatus="loop">
                                            <tr> 
                                                ...
                                                <td class="action-buttons">
                                                    <a href="postdetail.jsp?id=${post.postID}">View</a>
                                                    <a href="editpost.jsp?id=${post.postID}">Edit</a>
                                                    <a href="postslist.jsp?deleteId=${post.postID}" onclick="return confirm('Are you sure you want to delete this post?')">Delete</a>
                                                </td>
                                            </tr>
                                        </c:forEach>
                                    </tbody>
                            ...
                            <div class="pagination-bx rounded-sm gray clearfix">
                                <ul class="pagination">
                                    <c:if test="${currentPage > 1}">
                                        <li class="previous">
                                            <a href="postslist.jsp?page=${currentPage - 1}${not empty param.search ? '&search='.concat(param.search) : ''}${not empty param.sortBy ? '&sortBy='.concat(param.sortBy) : ''}${not empty param.category ? '&category='.concat(param.category) : ''}${not empty param.author ? '&author='.concat(param.author) : ''}${not empty param.status ? '&status='.concat(param.status) : ''}${not empty param.feature ? '&feature='.concat(param.feature) : ''}">
                                    ...
                                    <c:forEach begin="1" end="${totalPages}" var="i">
                                        <li class="${currentPage == i ? 'active' : ''}">
                                            <a href="postslist.jsp?page=${i}${not empty param.search ? '&search='.concat(param.search) : ''}${not empty param.sortBy ? '&sortBy='.concat(param.sortBy) : ''}${not empty param.category ? '&category='.concat(param.category) : ''}${not empty param.author ? '&author='.concat(param.author) : ''}${not empty param.status ? '&status='.concat(param.status) : ''}${not empty param.feature ? '&feature='.concat(param.feature) : ''}">${i}</a>
                                        </li>
                                    </c:forEach>
